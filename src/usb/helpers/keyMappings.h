...
inline static uint16_t linearSearchKey(const KeyMap *keys, size_t count, String keyName)
{
    for (size_t i = 0; i < count; ++i)
    {
        if (String(keys[i].name).equals(keyName))
            return keys[i].value;
    }
    return 0;
}

inline uint16_t getKeyValue(String keyName)
{
    if (keyName.length() == 1)
        return keyName[0];
